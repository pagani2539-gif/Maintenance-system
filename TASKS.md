...
### ⚪ Upcoming / Todo
- [x] **Responsive Production Release Gate & Cross-Page Hardening**: Added a shared mobile layout contract for page padding, dense forms, master-detail layouts, dialogs, raw tables, touch actions, safe-area support, and 200% zoom. Restored full mobile-card action parity, protected admin-only Settings/Technicians routes, added Playwright viewport sweeps with CI artifacts, and documented authenticated plus real-device QA in `docs/RESPONSIVE_QA.md`. 🟢
